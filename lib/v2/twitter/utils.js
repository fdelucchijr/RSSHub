...
const ProcessFeed = async (ctx, { data = [] }, params = {}) => {
    ...
    const formatMedia = (item) => {
        ...
        item.extended_entities && item.extended_entities.media?.forEach((v) => iterateMedia(v));
        item.entities && item.entities.media?.forEach((v) => iterateMedia(v));
    ...
    return await Promise.all(
        data.map(async (item) => {
            ...
            return {
                title,
                author: `https://twitter.com/${item.user.id_str}`,
                description,
